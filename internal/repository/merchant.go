...
func (r repository) MerchantOmzetReport(userID int64, month, year, page, perPage int) (report []*models.MerchantMonthReport, totalRow int64, err error) {
	offset := (page - 1) * perPage
	firstOfMonth := time.Date(year, time.Month(month), 1, 0, 0, 0, 0, time.Local)
	lastOfMonth := firstOfMonth.AddDate(0, 1, -1)
	dbResp := r.db.Table("Transactions").Select("r.tgl, r.merchant_name, COALESCE(SUM(Transactions.bill_total),0) as omzet").Joins(`
        ...
        WHERE r.user_id = ?
        GROUP by r.tgl, r.user_id, r.merchant_id, r.merchant_name`, firstOfMonth.Format("2006-01-02"), int(lastOfMonth.Sub(firstOfMonth).Hours()/24), userID)
	...
	for rows.Next() {
		temp := &models.MerchantMonthReport{}
		err = rows.Scan(
			&temp.Date,
			&temp.MerchantName,
			&temp.Omzet)
